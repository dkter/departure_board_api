...
#[get("/departures-after-time/{time}/{lat}/{lon}/{limit}")]
async fn get_departures_after_time(path: web::Path<(u32, f64, f64, u64)>, data: web::Data<AppState>) -> error::Result<web::Json<Vec<FormattedData>>> {
    let db_client = data.pool.get().await?;
    let (time, lat, lon, limit) = path.into_inner();
    let now = gtfs::GtfsTime::from(time);
